...
export const artworkLogSchema = z.object({
  ...
  JobID: z.string().optional().nullable(), // Now optional, conditional validation
  Category: z.string().min(1, { message: "Category is required." }),
  ...
  } else { // Project, Lead, Other
    if (!data.JobID) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Job ID is required for 'Project', 'Lead', or 'Other' categories.",
        path: ["JobID"],
      });
    }
    if (data.RequesterDepartment) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Requester Department should not be set for this category.",
        path: ["RequesterDepartment"],
      });
    }
  }
